...
class AutoRegressorModule(BaseLightningModule):
    ...
    def forward(self, input: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        log_step = (
            self.global_step
            if self.training and hasattr(self, "_log_weights") and self._log_weights
            else None
        )
    ...
    def step(self, batch: dict[str, Any], phase: str) -> torch.Tensor | None:
        ...
        if self._log_weights and hasattr(self.model, "set_tensorboard_writer"):
            try:
                ...
                self.model.set_tensorboard_writer(tensorboard)  # type: ignore
            except Exception:
                pass
    ...
    def _log_eval_post_activations(self, input_seq: torch.Tensor, tensorboard) -> None:
        ...
        try:
            with torch.no_grad():
                result = self.model(
                    input_seq, 1, certainty_stop=True, return_post_activations=True
                )
                if isinstance(result, tuple):
                    ...
                    for tick_idx, post_act in enumerate(post_activations):
                        ...
                        tensorboard.add_histogram(
                            f"{prefix}/post_activations", post_act, self._val_count
                        )
                        tensorboard.add_scalar(
                            f"{prefix}/post_activations_mean",
                            post_act.mean(),
                            self._val_count,
                        )
                        tensorboard.add_scalar(
                            f"{prefix}/post_activations_std",
                            post_act.std(),
                            self._val_count,
                        )
                        tensorboard.add_scalar(
                            f"{prefix}/post_activations_max",
                            post_act.max(),
                            self._val_count,
                        )
                        tensorboard.add_scalar(
                            f"{prefix}/post_activations_min",
                            post_act.min(),
                            self._val_count,
                        )
                        ...
                        tensorboard.add_histogram(
                            f"{prefix}/per_neuron_mean",
                            per_neuron_mean,
                            self._val_count,
                        )
                        tensorboard.add_histogram(
                            f"{prefix}/per_neuron_std", per_neuron_std, self._val_count
                        )

                    all_post_acts = torch.stack(post_activations, dim=0)
                    tensorboard.add_histogram(
                        "eval/all_ticks/post_activations",
                        all_post_acts,
                        self._val_count,
                    )
                    tensorboard.add_scalar(
                        "eval/num_ticks", len(post_activations), self._val_count
                    )

        except Exception as e:
            warnings.warn(f"Failed to log eval post activations: {e}")
